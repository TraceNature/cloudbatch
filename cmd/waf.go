...
// waf域名配置参数个数
const addWafDomainMaxParameter = 14
...
func excelWaf2interface(row []string) *models.AddDomain {
	...
	// 映射回源服务器https端口
	if row[8] == "" {
		domain.RsConfig.HttpsRsPort = []string{}
	} else {
	...
	// 映射回源服务器http端口
	if row[13] == "" {
		domain.RsConfig.HttpRsPort = []string{}
	} else {
		httpRsPort := strings.Split(row[13], ",")
		domain.RsConfig.HttpRsPort = httpRsPort
	}
	// 映射是否已使用代理
	switch row[9] {
	case "是":
